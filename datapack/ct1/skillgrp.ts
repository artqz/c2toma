...
export function loadSkillGrpCT1(): SkillGrpEntry[] {
  const skillsRaw = parseCsv(
    Fs.readFileSync("datapack/ct1/Skillgrp.txt", "utf8"),
  ...
  return skills
    // .filter((x) => x.skill_id < "10000") // тома пидор (убираем дроп и спойл из скилзов)
    .map((x) => {
      testMap.set(x.oper_type, parseInt(x.skill_id));
      return {

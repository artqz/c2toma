  ...
  for (const skillName of loadSkillNamesC6()) {
    ...
    if (skillGF) {
      ...
      if (skillName.level >= 141 && skillName.level <= 170) {
        const skillGF2 = skillGFById.get(
          skillName.id + "_" + (skillName.level + 60)
        );
        if (skillGF2) {
...
function getRuNames(deps: { skills: Map<string, Skill> }) {
  const skillNamesGF = new Map(
    loadSkillNamesGF().map((s) => [s.id + "_" + s.level, s])
  );
  ...
  for (const skill of deps.skills.values()) {
    ...
    if (skillNameGF) {
      ...
      if (skill.level! >= 141 && skill.level! <= 170) {
        const skillNameGF = skillNamesGF.get(
          skill.id + "_" + (skill.level! + 60)
        );
        if (skillNameGF) {
          skill.name.ru = skillNameGF.name.ru;
        } else {
...
function addSkill(skillData: SkillEntryGF, skillName: SkillNameEntry) {
